...
class DataSet():
    ...
    def __init__(self, filepath):
        # ensure it works in different working folders
        baseDir = os.getcwd()
        if os.path.basename(baseDir) != 'scripts':
            self.filepath = os.path.join(baseDir, filepath)
        else:
            self.filepath = os.path.join(dirname(baseDir), filepath)
...
if __name__ == '__main__':
    # define the paths
    csv_path = 'datas/01-medianIncome/ACS_Median_Household_Income_Variables_-_Boundaries/Tract_2.csv'
    ...
    out_path = 'datas/01income-cleaned.csv'
